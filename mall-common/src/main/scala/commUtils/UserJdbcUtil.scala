...
object UserJdbcUtil {
  ...
  def init()={
    ...
    val config = UserConfigUtil("config.properties").config
...
//    UserJdbcUtil.executeBatchUpdate("insert into page_convert_rate values(?,?,?)",Array(Array("1","1_2",1.2),Array("2","2_3",0.8)))
//  }
}
